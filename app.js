...
exec('protractor s2k_entities_conf.js > "log/' + filename + '"', {
  cwd: '/root/s2k_ui_test/bat'
}, function(error, stdout, stderr) {
    if ( !error ) {
        console.log('UI TEST ["Entities"] MODULE FINISED. \nPLEASE CHECK THE LOG FILE: "' + filename + '"');
        //console.log(stdout);
    } else {
        console.log('UI TEST ["Entities"] MODULE ERROR. \nPLEASE CHECK THE LOG FILE: "' + filename + '"');
        console.log(error);
    }
});

filename = moment().format('YYYY-MM-DD HH-mm-ss.SSS') + '[MRK].log';
